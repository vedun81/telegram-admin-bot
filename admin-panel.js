...
function addQuestionRow() {
  ...
  wrap.innerHTML = `
    ...
    <input type="text" placeholder="Текст питання" class="q-text">
    <input type="text" placeholder="Правильна відповідь" class="q-correct">
    <input type="text" placeholder="Неправильна відповідь 1" class="q-wrong">
    <input type="text" placeholder="Неправильна відповідь 2" class="q-wrong">
    <input type="text" placeholder="Неправильна відповідь 3 (необов'язково)" class="q-wrong">
  `;
...
async function saveTest() {
  ...
  try {
    await rtdb.ref('customTests').push({
      name, questions,
...
async function loadTestsList() {
  ...
  container.innerHTML = Object.entries(data).map(([id, t]) => `
    <div class="list-item">
      <span>${escapeHtml(t.name)} <span style="color:#aaa;font-size:11px;">(${(t.questions || []).length} питань)</span></span>
      <button class="btn btn-danger" onclick="deleteTest('${id}')">🗑️</button>
    </div>
  `).join('');
}
