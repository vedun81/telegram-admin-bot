...
async function loadLogs() {
  ...
  container.innerHTML = 'Завантаження...';
  let data = null;
  try { data = await dbGet('logs'); } catch (e) { console.error(e); }
  ...
  let all = [];
  for (const uid in data) {
    for (const logId in data[uid]) all.push(data[uid][logId]);
  }
  all.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
  renderLogs(all);
}
...
function renderLogs(list) {
  ...
  container.innerHTML = list.slice(0, 300).map(l => `
    <div class="log-entry">
      <span class="log-player">${LOG_ICONS[l.type] || '📌'} ${escapeHtml(l.nick || '?')}</span>
      — ${escapeHtml(l.details || l.type || '')}
      <div style="font-size:11px;color:#aaa;margin-top:2px;">${escapeHtml(l.date || '')}</div>
    </div>
  `).join('');
}
...
function searchLogs() {
  ...
  dbGet('logs').then(data => {
    if (!data) return;
    let all = [];
    for (const uid in data) for (const logId in data[uid]) all.push(data[uid][logId]);
    all.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
    if (q) all = all.filter(l => (l.nick || '').toLowerCase().includes(q));
    renderLogs(all);
  });
}
